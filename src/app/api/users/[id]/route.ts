...
export async function GET(
  ...
  try {
    ...
    console.log(
      "Connected to Database: find_all_database, now getting user data.",
    );

    // Get Data
    let user = await db.collection("find_all_users").findOne({ clerkId });
    ...
    return NextResponse.json(
      { error: "Unable to manage request" },
      { status: 400 },
    );
  }
}
...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Unable to post item request" },
      { status: 400 },
    );
  }
}
...
export async function PUT(
  ...
  try {
    const clerkId = (await params).id;
    const { originalItemName, editItemName, editItemLocation } =
      await req.json();
    console.log(`Updating item ${originalItemName} for user: ${clerkId}`);
    ...
    // Use MongoDB Positional Operator $
    const result = await collection.updateOne(
      {
        clerkId: clerkId,
        "items.name": originalItemName,
      },
      {
        $set: {
          "items.$.name": editItemName,
          "items.$.location": editItemLocation,
    ...
    console.log(`${originalItemName} successfully`);
